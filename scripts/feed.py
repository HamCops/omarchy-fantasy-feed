...
PARSER_VERSION = "espn-narrative-v3"
...
_INTERCEPTION_PATTERN = re.compile(
    ...
    rf"(?: at {_TEAM} \d+)?\."
    rf"(?: (?P=defender) {_RUSH_LOCATION}for -?\d+ yards?{_TACKLER}\.)?"
)
...
_ELIGIBLE_PREAMBLE = re.compile(r"^(?:[A-Z][a-z]?\.[A-Za-z.'\- ]+? reported in as eligible\.\s*)+")
